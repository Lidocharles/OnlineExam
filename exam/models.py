...
class Question(models.Model):
    ...
    def get_answer(self):
        if self.format == 'TF':
            ...
            return self.correct_answer.lower() if self.correct_answer else None

    def total_correct_answers(self):
        if self.format == 'FIB':
            return StudentAnswer.objects.filter(question=self).filter(answer__iexact=self.get_answer()).count()
        return StudentAnswer.objects.filter(question=self, answer=self.get_answer()).count()

    def total_wrong_answers(self):
        if self.format == 'FIB':
            return StudentAnswer.objects.filter(question=self).exclude(answer__iexact=self.get_answer()).count()
